...
function startApp() {
    ...
    log("Iniciando Conexão...", "blue");
    
    ws = new WebSocket(url);

    ws.onopen = () => {
        log("WebSocket OK!", "green");
        ...
        .catch(err => log("ERRO Câmera: " + err.message, "red"));
    };

    ws.onerror = (e) => log("ERRO de Rede/WebSocket", "red");
    ws.onclose = () => log("Conexão Fechada", "orange");
}
...
function setupLoop() {
    ...
    ws.onmessage = (e) => {
        try {
            ...
            if(res.error) {
                log("ERRO NO SERVIDOR: " + res.error, "red");
                return;
            }
            let img = new Image();
            img.onload = () => {
                ...
                res.detections.forEach(d => {
                    ...
                    ctx.fillText(d.prediction, d.x * s, d.y * s + 20);
                });
            };
            img.src = res.image;
        } catch(err) {
            log("ERRO DADOS: " + e.data.substring(0, 50), "red");
        }
    };
}
"""
...
def get():
    return Title("Debugger Vision"), Main(
        ...
        Div(id="logger", style="height: 200px; overflow-y: auto; background: #eee; padding: 10px; border-top: 2px solid #333; font-family: monospace;"),
        Script(JS_CODE)
    )
                    ...
                    try:
                        ...
                        if frame is not None:
                            ...
                            await send({'type': 'websocket.send', 'text': json.dumps({
                                ...
                                "detections": processor.get_detections(frame, res, loader.clf)
                            })})
                    except Exception as inner_e:
                        await send({'type': 'websocket.send', 'text': json.dumps({"error": str(inner_e)})})
    except Exception as outer_e:
        await send({'type': 'websocket.send', 'text': json.dumps({"error": f"Erro Inicial: {str(outer_e)}"})})
